...
function repeat_F() {
    var orig_f = document.getElementById('orig_f');
    if (!orig_f) {
        console.error('Original element not found');
        return;
    }
    var clone = orig_f.cloneNode(true);
    var repeat_Fiction = document.getElementById('repeat_Fiction');
    if (!repeat_Fiction) {
        console.error('Target container not found');
        return;
    }
    repeat_Fiction.appendChild(clone);
    console.log("Function loaded");
};
...
{
    var data = document.getElementById('ongoing_input').value;
    localStorage.setItem('myData', 'data');
    var storedData = localStorage.getItem('myData');
    if (storedData) {
        document.getElementById('display_ongoing').textContent = storedData;
    } else {
        document.getElementById('display_ongoing').textContent = 'No data found';
    }
    document.addEventListener('DOMContentLoaded', function() {});    
};